...
- Dark neon CSS in `web/static/css/app.css`. Site-wide `background.png`. Visitor: sofa hero + Pigeon Coop/Discord/Twitch/YouTube + Hub square (Pigeon `discord.gg/gXnpmM8zDX`, Discord `discord.gg/ZdjTbeMEt7`). Sesh Hub (logged-in): folded bar + avatar; same four social icons, no Hub square. Browser title suffix **Sesh Hub**. `/login` is the OAuth picker. Well is 70% / min 720px (100% under 640px). Queue list actions sit on the right. Public nav: **FS Team** then **Friends**; **Photos** after Videos.

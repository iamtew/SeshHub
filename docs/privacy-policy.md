...
| Post your roster name, Twitch login, stream title, and twitch.tv link into that Discord channel when you go live | So the Sesh Sofa Discord can see who is streaming | Art. 6(1)(f) - legitimate interest in community notices, only while that announcement is switched on, and only if you saved a Twitch login |
...
- **Bot settings:** the announce channel ID, the home channel ID, the on/off switches, and the Twitch going-live message template, kept until an administrator changes them. They identify channels, not a person.

...
**What:** a public slug, your skater name, your real name, a biography, your stance, a roster status, avatar and banner URLs, optional **corner-radius values**, a **border style name** (none, a solid default or custom colour, or a named animation), and for a custom colour an optional **hex colour**, optional **border width** (pixels) and **border blur** (pixels, default 0), your location, sponsors, social links, signature tricks, a featured video ID, an optional **Twitch login**, an optional **last stream title** (kept after you go offline), and **when the stream started** while you are live, and **former slugs** (only while nobody else is using that URL). If you upload a site photo, we also store a re-encoded JPEG file on the same Amsterdam server (`data/avatars/{id}.jpg`, shown at `/media/avatars/{id}.jpg`). The original upload is not kept. We strip metadata by re-encoding. Maximum upload size is 15 MB; we store a square JPEG at most 1024 pixels on a side. You may also upload a public gallery of up to 10 photos; those files are stored as described in section 4.10.

**Why:** to show the FS Team roster at `/team` and the Friends roster at `/friends`, and your own profile page at `/team/{slug}` or `/friends/{slug}`. Former slugs 302 to your current page so old links keep working, until that slug is claimed again. The Twitch login is so we can show whether you are live and post a Discord notice when a stream starts.
...
**Read: this profile is public.** Anyone on the internet, signed in or not, can see it. Note in particular that the name shown publicly is your **display name (stored as real name) if you have filled it in**, and falls back to your skater name only if you have not. Your **slug** is the public URL `/team/{slug}` (FS Team) or `/friends/{slug}` (Friends). Former slugs are public too: visiting them redirects to your current page. Your **location**, **biography**, **profile photo** (site upload or Discord/YouTube fallback, plus the frame shape and any border style), **gallery photos**, and **social URLs** (shown as public buttons; plus a YouTube channel button when YouTube is linked, and a Twitch button when a Twitch login is set) are also shown publicly when set. If you set a Twitch login, that name is public. When you are shown as live (Helix, or a short superadmin website audition), the **last stream title** is public on `/team`, `/friends`, your roster page, and site-wide banners under the menu, with a link to `https://www.twitch.tv/{login}`. Please do not put anything in these fields that you would not want a stranger to read.

**Updated:** by you, at `/dashboard/profile`. That form edits your display name, slug, biography, stance, location, social URLs (Links tab, up to 10 `http`/`https` addresses), **Twitch login** (Twitch tab), featured video, YouTube Feed Filter, photo frame (corner roundness, border style, optional custom hex colour, border width, and border blur), and an optional site-only profile photo. Opening the Twitch tab after a login is saved asks Helix for that channel’s public display name, description, created date, and whether it is live, so you can see the lookup worked. Those fields are shown to you on the dashboard and are not stored, except the live title and start time already described. Gallery photos are added and deleted at `/dashboard/gallery`, and added when you @mention the Discord bot with JPEG or PNG attachments. Changing your slug records the previous one as a redirect and frees it for anyone to claim later. Logging in still refreshes your Discord skater name and the provider avatar URL on your account record; it does not overwrite a slug you chose, and it does not overwrite a site photo you uploaded. Clearing the site photo falls back to Discord or YouTube. An administrator can separately change an FS Team roster status (Active, Pro, and so on). Discord-linked Friends show as Friend. Queue-approved Friends with no Discord ID show as **Sesh Hub Friend**. If Twitch app credentials are configured, the server asks Helix about once a minute whether saved logins are live and writes or clears the title and start time. Changing or clearing the Twitch login clears that snapshot.

**Deleted:** when your account is deleted, or by an administrator. Former-slug redirects are deleted with the profile. The site photo file and any gallery photo files are deleted with the profile. If someone else takes an old slug, that redirect row is deleted so their page wins. The Twitch login and live snapshot go with the profile. Clearing the Twitch login on `/dashboard/profile` deletes the login and snapshot without deleting the profile.
...
**Created:** when you add a JPEG or PNG on `/dashboard/gallery`, while you still have fewer than 10 photos; or when you @mention the Discord bot (and you have a public roster profile) with JPEG or PNG attachments on that message, or, if that mention has none, on the nearest of your previous 3 messages in the same channel that has such an attachment. The original Discord file is not kept. We re-encode as on the dashboard. If the gallery already has 10 photos, we delete the oldest (by created time) so the new one can be stored.
...
**Deleted:** when you delete a photo on `/dashboard/gallery`, when a Discord ingest would exceed 10 photos (the oldest is removed), when your account is deleted, or when an administrator deletes the profile. The database row and the JPEG file both go.
...
**What:** up to 3 attachment records per forum post: an internal file id, the post id, a sort position, a kind (`image`, `audio`, `video`, or `pdf`), a file extension, a MIME type, and the **original filename** (basename you chose at upload; we keep our stored file extension if we re-encoded JPEG/PNG). Files live on the same Amsterdam server at `data/forum/{id}.{ext}`, shown at `/media/forum/{id}.{ext}` only while you are signed in. Downloads use that original filename. JPEG and PNG are re-encoded to JPEG (original not kept; we strip metadata by re-encoding; long edge at most 1600 pixels). GIF, WebP, audio, video, and PDF are stored as uploaded, so embedded metadata in those files may remain. Maximum upload size is 15 MB per file. We accept JPEG, PNG, GIF, WebP, MP3, WAV, OGG, FLAC, M4A, MP4, WebM, and PDF. Other types are refused.
...
- **Discord's API**, during sign-in, to read your profile and your roles in the Sesh Sofa server. When an administrator has switched an announcement on at `/admin/bot` and set an announce channel, the server also keeps a bot session on that API and posts into that channel: the title and public URL of a news article the first time it is published, and only when the article is public; the title and hub URL of a forum thread you start (not the post body, attachments, or your name); and your display name and username when you submit an access request. If the Twitch going-live switch is on and a **Twitch announce channel** is set, the bot posts your roster display name, Twitch login, stream title, and `https://www.twitch.tv/{login}` link there when a saved Twitch name goes from offline to live. If that channel is a Discord Announcement channel, the bot also crossposts the message so servers following that channel receive it. When the forum @mention switch is on, an @mention of an account that has Discord linked is posted as a Discord ping of that person and of the author (the author's display name if they have no Discord id), plus the thread title and URL. The post body is not included, and an account without Discord linked is not pinged. Editing a post pings only mentions that were not already there. Each of those stays off until an administrator enables it. When an administrator sets a **home channel**, Discord also delivers guild messages to that session (message content intent). The bot uses a message only if it is in that home channel, or if it @mentions the bot or replies to the bot in another channel. It does not store message text. Until a home channel is set, it does not use those messages. If you @mention the bot and you have a public roster profile on this Service, it may download JPEG or PNG attachments on that message (or, if there are none, on the nearest of your previous 3 messages in that channel that has such an attachment) from Discord's CDN (`cdn.discordapp.com` or `media.discordapp.net`), re-encode them as gallery photos, and discard the Discord URL. Those lookback messages are not stored. GIF, video, and other types are ignored. `/admin/bot` shows a short in-memory list of posts, connect and disconnect events, and gallery @mention outcomes (skip reason, Discord user id of the person who tagged the bot, channel id, photo count, or a reaction error). Message text is not stored. That list is not written to the database and is gone when the process stops. The bot token stays in the server environment, not in the database.
...
- **Twitch Helix**, if `TWITCH_CLIENT_ID` and `TWITCH_CLIENT_SECRET` are set, to mint an app access token (not a user token) and to read public stream status for Twitch logins saved on roster profiles (channel lookup on save and when you open `/dashboard/profile` with a login set; live streams about once a minute; last channel title when a login is not live). The dashboard preview uses public display name, description, created date, and live title/game; we do not store those extra fields. We store the login you typed plus the last public stream title, and the start time while live. We do not store a Twitch user OAuth token. Our server’s IP is sent, not yours. Clicking a live link loads `twitch.tv` in your browser, which is Twitch’s own site.
- **decapi.me**, only as a fallback when Helix does not return a title, to read the public stream title for a saved Twitch login. The request is our server’s IP and the login; not yours.
...
| Post your roster name, Twitch login, stream title, and twitch.tv link into a Discord channel an administrator chose for Twitch, and crosspost it when that channel is an Announcement channel | So the Sesh Sofa Discord (and servers following that channel) can see who is streaming | Art. 6(1)(f) - legitimate interest in community notices, only while that announcement is switched on, and only if you saved a Twitch login |
| Ask Twitch Helix (and if needed decapi.me) for public live status and last stream title, and show those on the roster and live banners | You put the login on your public profile | Art. 6(1)(f), with your role in publishing it |
| Show Helix display name, description, created date, and live/offline on the Twitch tab of `/dashboard/profile` | So you can see the login resolved | Art. 6(1)(f). Display only; not stored except the live snapshot |
...
| Copy JPEG or PNG attachments from a Discord message where you @mentioned the bot onto your public gallery | You asked the bot to publish those photos | Art. 6(1)(f), with your role in publishing it. Same public gallery as `/dashboard/gallery` |
...
- **Skater profile:** kept until the account is deleted, or an administrator removes the profile. A site photo file and gallery photo files are kept for the same time and deleted with the profile. A Twitch stream title is overwritten about once a minute while you are live, and the last title is kept when Helix says you are offline. The start time is cleared when you go offline or you clear the login.
...
- **Bot settings:** the announce channel ID, the home channel ID, the Twitch announce channel ID, the on/off switches, and the Twitch going-live message template, kept until an administrator changes them. They identify channels, not a person.
...
**How to exercise these rights.** Most of them are on `/account` while you are signed in: unlink a provider, download a JSON export of your account, profile (including photo URL and frame settings), gallery photo URLs, former slugs, YouTube Feed Filter, access request, Forum posts (thread title plus markdown body and attachment URLs and original filenames), and Forum mentions, or delete the account. Logging out ends every session. You can also email the contact address in section 1. We will respond within one month, as Art. 12(3) requires.

- **Access or portability:** use **Download my data** on `/account`, or ask us by email. The export includes the public URL of a site photo if you uploaded one, the public URLs of gallery photos, and the signed-in URLs and original filenames of your forum attachments, not the file bytes.
...
- **Rectification:** most profile fields update themselves from Discord or YouTube on your next sign-in. Your display name, slug, biography, location, social URLs, Twitch login (Twitch tab), featured clip, YouTube Feed Filter, site photo and photo frame (including border style, width, and blur) are yours to edit at `/dashboard/profile`. Gallery photos are yours to add and delete at `/dashboard/gallery`, and you can add them by @mentioning the Discord bot with JPEG or PNG attachments.

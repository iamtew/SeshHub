...
- **Twitch Helix**, if `TWITCH_CLIENT_ID` and `TWITCH_CLIENT_SECRET` are set, to mint an app access token (not a user token) and to read public stream status for Twitch logins saved on roster profiles (channel lookup on save and when you open `/dashboard/profile` with a login set; live streams about once a minute). The dashboard preview uses public display name, description, created date, and live title/game; we do not store those extra fields. We store the login you typed plus, while live, the public title and start time. We do not store a Twitch user OAuth token. Our server’s IP is sent, not yours. Clicking a live link loads `twitch.tv` in your browser, which is Twitch’s own site.
...
| Ask Twitch Helix whether a saved login is live, and show title and uptime on the public roster | You put the login on your public profile | Art. 6(1)(f), with your role in publishing it |
...
- **Skater profile:** kept until the account is deleted, or an administrator removes the profile. A site photo file and gallery photo files are kept for the same time and deleted with the profile. A Twitch live title and start time are overwritten about once a minute while you are live, and cleared when Helix says you are offline or you clear the login.
